...
describe("estimateCreateModal3 unit price cycle label (Core 0.1)", () => {
  const proto = EstimateCreateModal3.prototype;
  const label = Object.getOwnPropertyDescriptor(proto, "unitPriceCycleLabel")
    .get;

  it("uses Japanese /月 and does not keep / month", () => {
    expect(label.call({})).toBe("/月");
    expect(label.call({})).not.toBe("/ month");
  });
});
